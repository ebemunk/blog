...
const castling = (data, totalGames) => {
  const chart = (data, node) => {
    ...
    const pieData = Object.entries(data).concat([
      ['No Castling', totalGames - d3sum(Object.values(data))],
    ])
    ...
    const colorScale = d3scaleOrdinal([
      d3schemeCategory10[0],
      d3schemeCategory10[1],
      'grey',
  ...
  d3selectAll('#castling-container div')
    .data(['White', 'Black', 'Side'])
    .each((d, i, n) => chart(data[d], n[i]))
}
